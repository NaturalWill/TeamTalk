...
void build_ios_push_flash(string &flash, uint32_t msg_type, uint32_t from_id) {
    ...
    if (pos_prefix != string::npos && pos_suffix != string::npos && pos_prefix < pos_suffix) {
        ...
        if (pImUser) {
            ...
            } else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_EXT) {
                jsonxx::Object o;
                if (o.parse(flash) && o.has<jsonxx::String>("type")) {
                    jsonxx::String& strType = o.get<jsonxx::String>("type");
                    if (!strType.empty()) {
                        msg_tmp.append(nick_name);
                        msg_tmp.append(strType.c_str());
                    }
                } else {
                    msg_tmp.append(nick_name);
                    msg_tmp.append("给您发送了一条自定义消息");
                }
            } else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_EXT) {
                jsonxx::Object o;
                if (o.parse(flash) && o.has<jsonxx::String>("type")) {
                    jsonxx::String& strType = o.get<jsonxx::String>("type");
                    if (!strType.empty()) {
